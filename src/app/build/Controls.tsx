...
export function Controls({ wheels, decks, metals, className }: Props) {
    ...
    useEffect(() => {
        ...
        if (isFilled.keyText(selectedWheel?.uid)) url.searchParams.set('wheel', selectedWheel.uid);
        if (isFilled.keyText(selectedDeck?.uid)) url.searchParams.set('deck', selectedDeck.uid);
        if (isFilled.keyText(selectedTruck?.uid)) url.searchParams.set('truck', selectedTruck.uid);
        if (isFilled.keyText(selectedBolt?.uid)) url.searchParams.set('bolt', selectedBolt.uid);
...
function Options({ children, selectedName, title }: OptionsProps) {
